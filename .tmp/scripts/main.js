...
var detectmob = function detectmob() {
  var max = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 800;
...
var collapseFunc = function collapseFunc(e) {
  if (detectmob()) {
...
var resetcontainers = function resetcontainers() {
...
var getNumberActive = function getNumberActive(activeElement) {
  var arrayOfStrings = activeElement.split('-'),
      numberActive = parseInt(arrayOfStrings[1]);
  return numberActive;
};

var setReadmoreMobile = function setReadmoreMobile() {
  if (detectmob(380)) {
...
var changeSrcImage = function changeSrcImage(activeElement) {

  var numberActive = getNumberActive(activeElement);

  var oldImage = 'previousimage cimg-' + (numberActive - 1).toString();
  var newImage = 'currentimage cimg-' + numberActive.toString();

  $('#aboutusContImg-1').removeClass().addClass(oldImage);
  $('#aboutusContImg-2').removeClass().addClass(newImage);
};

var executeSliderAboutus = function executeSliderAboutus() {
  var activeElement = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
...
var resetCollapse = function resetCollapse() {
  var cch = '10.94rem';
  var bimg = '50rem';
...
//Slider things
var clearli = function clearli() {
  console.log('clearli function');
  $('#coachesslider li').each(function (i, e) {
    var classOfLi = $(this).attr('class'); // This is your rel value
    var isClone = classOfLi.search('bx-clone') !== -1 ? true : false;
    if (isClone) {
...
var displayContentSlider = function displayContentSlider(num) {
  num = num + 1;
  var el = '.content-slider div:nth-child(' + num + ')';
  var count = $('.content-slider div').length;
  $('.content-slider div').removeClass('active-content');
  $(el).addClass('active-content');
};

var existEmpty = function existEmpty() {
  $('#coachesslider li').each(function (i, e) {
    var classOfLi = $(this).attr('class'); // This is your rel value
